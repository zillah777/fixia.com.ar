...
async function main() {
  console.log('🌱 Seeding categories in batches...');
  ...
  console.log(`Creating ${categories.length} categories in batches of 10...`);
  ...
  for (let i = 0; i < categories.length; i += BATCH_SIZE) {
    ...
    for (const category of batch) {
      try {
        ...
        if (result.created_at.getTime() === result.created_at.getTime()) {
          created++;
        } else {
          updated++;
        }
    ...
    // Wait 500ms between batches to avoid overwhelming the connection
